      ...
      if (!IsEnb ())
        {
          double antennaGainLinear = std::pow (10.0, 
                ...
                (ConstCast<ThreeGppAntennaArrayModel> (GetAntennaArray ()))->GetGainDb ())/10.0);
          ...
          if (dlCtrlRxParams->cellId == GetCellId () || 
             (dlCtrlRxParams->isSSB && m_IAcontinues))
            {
    ...
    case IDLE:
      {
        ...
        {
          txParams->isSSB = txParams->isCSIRS = false;
        }        
...
NrSpectrumPhy::StartRxDlCtrl (const Ptr<NrSpectrumSignalParametersDlCtrlFrame>& params)
{
  ...
  NS_LOG_FUNCTION (this);
  if ((params->cellId == GetCellId () && !IsEnb ()) || (params->isSSB && m_IAcontinues));
  {
