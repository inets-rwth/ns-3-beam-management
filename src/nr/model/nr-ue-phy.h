...
public:
  ...
  void SetSSBRLMScanScenario (SSBRLMScanScenario ssbRLMScanScenario);
...
private:
  ...
  std::pair<uint8_t, BeamId> RetrieveOptimalGnbFromMap (std::map<uint8_t, std::vector<std::pair<std::pair<SfnSf, uint16_t>, std::pair<double, BeamId>>>>);
  ...
  Ptr<SSBProcessor> m_ssbRLMProcessor;
  ...
  std::map<SfnSfKey, std::pair<BeamId, std::pair<BeamId,double>>> m_recvCSIMap;
  std::vector<std::pair<SfnSfKey, std::pair<BeamId, double>>> m_subOptimalCSIBeamMap;
