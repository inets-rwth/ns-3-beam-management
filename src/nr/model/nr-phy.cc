...
NrPhy::GetTypeId ()
{
  static TypeId
    ...
    TypeId ("ns3::NrPhy")
    .SetParent<Object> ()
    .AddAttribute ("SSBRLMScanScenario",
                   "Scenario for RX scanning used for SSB RLM procedure",
                   EnumValue (SSBRLMScanScenario::ScanAllRXDirections),
                   MakeEnumAccessor (&NrPhy::m_ssbRlmScanScenario),
                   MakeEnumChecker (ScanBestCSIRLMDirections, "BestCSIRLMDirections",
                                    ScanAllRXDirections, "AllRXDirections"))
...
NrPhy::GetAodAoaFromSM (Vector enbLoc)
{
 return DynamicCast<ThreeGppSpectrumPropagationLossModel>(m_spectrumPropagationLossModel)->GetAodAoaFromCM (enbLoc);
}
