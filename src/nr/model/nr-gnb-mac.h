...
public:
  ...
  */
  ...
  void DoForwardIAStateToSched (bool iaPerformed);

  void DoSetCSIRSFlag (bool csiRSFlag);
...
private:
  ...
  std::pair<bool, std::pair<uint64_t, uint8_t>> IsCSIRSRequired (const SfnSf &dlSfn);

  void DoSetCSIRSRLMResources (uint16_t csiRSPeriodicity, uint16_t csiRSOffset, uint8_t maxCSIRSResource, uint64_t imsi);
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  std::map<uint64_t,std::map<uint8_t, std::vector<std::pair<uint8_t, uint8_t>>>> m_mapOfCSIRSResources;
