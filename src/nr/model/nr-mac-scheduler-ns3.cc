                                   ...
                                   const std::vector <DlHarqInfo> &dlHarqFeedback)
{
  ...
  std::pair<bool, std::pair<uint64_t, uint8_t>> boolImsiPair = m_macSchedSapUser->IsCSIRSRequired (params.m_snfSf);
  if (boolImsiPair.first)
  {
...
NrMacSchedulerNs3::DoSchedDlTriggerReq (const NrMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  if ((params.m_dlHarqInfoList.size () > 0 || m_dlHarqToRetransmit.size () > 0) && !IAisPerformed)
    {
...
void
NrMacSchedulerNs3::DoSetIAStateOfMacSched (bool iaPerformed)
{
