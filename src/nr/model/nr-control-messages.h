...
class NRCSIReportMessage : public NrControlMessage
{
  ...
  void SetCSIRSComponent (uint8_t csiRSRank, std::pair<SfnSfKey, std::pair<BeamId, uint32_t>> csiRSMap);

  std::pair<SfnSfKey, std::pair<BeamId, uint32_t>> GetCSIRSMap (uint8_t csiRSRank);

  private:

  std::pair<SfnSfKey, std::pair<BeamId, uint32_t>> m_firstCSIResource;
  std::pair<SfnSfKey, std::pair<BeamId, uint32_t>> m_secondCSIResource;
  std::pair<SfnSfKey, std::pair<BeamId, uint32_t>> m_thirdCSIResource;
  std::pair<SfnSfKey, std::pair<BeamId, uint32_t>> m_fourthCSIResource;
  uint8_t m_cellId;
  uint16_t m_rnti;
};
