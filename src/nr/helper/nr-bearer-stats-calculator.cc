...
NrBearerStatsCalculator::DlRxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid, uint32_t packetSize, uint64_t delay)
{
  ...
  if (m_protocolType == "PDCP")
  {
    ...
    if (((Simulator::Now ().GetSeconds () - timeLastOutput) > 0.05))
    {
      ...
      m_dlPdcpThruFile << Simulator::Now ().GetSeconds () << " " << cellId << " " << rnti << " "
                           << (uint32_t) lcid << " " << totalBytesReceived * 8 / (Simulator::Now ().GetSeconds ()- timeLastOutput) << " " << std::endl;
    ...
    {
      // NS_LOG_UNCOND("rlc");
      totalBytesReceivedRlc += packetSize;
      if (((Simulator::Now ().GetSeconds () - timeLastOutputRlc) > 0.5))
        {
          ...
          m_dlRlcThruFile << Simulator::Now ().GetSeconds () << " " << cellId << " " << rnti << " "
                           << (uint32_t) lcid << " " << totalBytesReceivedRlc * 8 / (Simulator::Now ().GetSeconds ()- timeLastOutputRlc) << " " << std::endl;

          timeLastOutputRlc = Simulator::Now ().GetSeconds ();
          totalBytesReceivedRlc = 0;
        }
    }
