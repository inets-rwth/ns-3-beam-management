                                           ...
                                           const Ipv4Address &enb2X2Address) const
{
  ...
  Ptr<NrGnbNetDevice> enb1NrNetDevice = enb1LteDev->GetObject<NrGnbNetDevice> ();
  Ptr<LteEnbNetDevice> enb1LteNetDevice = enb1LteDev->GetObject<LteEnbNetDevice> ();
  // we may have a LTE or a MmWave eNB
  uint16_t enb1CellId = 0;
  if (enb1NrNetDevice != 0)
  {
    enb1CellId = enb1NrNetDevice->GetCellId ();
    NS_LOG_INFO ("MmWaveEnbNetDevice #1 = " << enb1NrNetDevice << " - CellId = " << enb1CellId);
  }
  else if (enb1LteNetDevice != 0)
  {
    enb1CellId = enb1LteNetDevice->GetCellId ();
    NS_LOG_INFO ("LteEnbNetDevice #1 = " << enb1LteNetDevice << " - CellId = " << enb1CellId);
  }

  Ptr<NrGnbNetDevice> enb2NrNetDevice = enb2LteDev->GetObject<NrGnbNetDevice> ();
  Ptr<LteEnbNetDevice> enb2LteNetDevice = enb2LteDev->GetObject<LteEnbNetDevice> ();
  // we may have a LTE or a MmWave eNB
  uint16_t enb2CellId = 0;
  if (enb2NrNetDevice != 0)
  {
    enb2CellId = enb2NrNetDevice->GetCellId ();
    NS_LOG_INFO ("MmWaveEnbNetDevice #2 = " << enb2NrNetDevice << " - CellId = " << enb2CellId);
    enb2NrNetDevice->GetRrc ()->AddX2Neighbour (enb1CellId);
  }
  else if (enb2LteNetDevice != 0)
  {
    enb2CellId = enb2LteNetDevice->GetCellId ();
    NS_LOG_INFO ("LteEnbNetDevice #2 = " << enb2LteNetDevice << " - CellId = " << enb2CellId);
    enb2LteNetDevice->GetRrc ()->AddX2Neighbour (enb1CellId);
  }

  //NS_ABORT_IF (enb1LteDevice == nullptr);
  //NS_ABORT_IF (enb2LteDevice == nullptr);

  NS_LOG_LOGIC ("LteEnbNetDevice #1 = " << enb1LteNetDevice << " - CellId = " << enb1CellId);
  NS_LOG_LOGIC ("LteEnbNetDevice #2 = " << enb2LteNetDevice << " - CellId = " << enb2CellId);
  ...
  if (enb1NrNetDevice != 0)
  {
    enb1NrNetDevice->GetRrc ()->AddX2Neighbour (enb2CellId);
  }
  else if(enb1LteNetDevice != 0)
  {
    enb1LteNetDevice->GetRrc ()->AddX2Neighbour (enb2CellId);
  }
  /*
