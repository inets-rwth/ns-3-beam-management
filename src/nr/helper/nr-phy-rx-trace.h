...
static void MacRxUe (Ptr<NrPhyRxTrace> phyStats, std::string path, uint16_t a, uint8_t b, uint32_t pktSize, SfnSf currentSlot);
...
private:
  ...
  static double timeLastMacRx;
  static uint32_t totalBytesMacRx;

  static std::ofstream m_rxThruFile;
  static std::string m_rxThruFilename;
  static double timeLastOutput;
  static uint32_t totalBytesReceived;

  static uint32_t lastFrameNum;
  static uint32_t lastFrameNumMacRx;
