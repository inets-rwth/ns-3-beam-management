...
private:
  ...
  void DoSetOptimalGnbBeamForImsi (uint64_t imsi, SfnSf startingSfn, std::vector<uint16_t> optimalBeamIndex, bool isMaxSNRCell);

  void DoAttachUeFromRRC (uint64_t imsi, Ptr<NetDevice> netDev);

  bool RegisterUe (uint64_t imsi);

  void DoDeregisterUeFromRRC (uint64_t imsi);

  void SetGnbIAState (bool IAperformed);

  void DoUpdateBeamsTBRLM (uint64_t imsi, std::vector<uint16_t> optimalBeamIndex, SfnSf startingSfn);
