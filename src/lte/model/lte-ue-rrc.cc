...
LteUeRrc::DoNotifyOutOfSyncNr ()
{
  ...
  if (m_sweepTriggerFromRrcReceived)
  {
    NS_LOG_UNCOND ("RadioLinkFaikureDetected: RLF detected, but a handover is scheduled");
...
void 
LteUeRrc::DoSendSSBRSBeamReport (std::vector<uint16_t> optimalBeamIndexVector, SfnSf startingSfn, uint8_t cellId)
{
  LteRrcSap::UpdateBeamsTbRLM params;
  params.ueImsi = GetImsi ();
  params.optimalBeamIndexVector = optimalBeamIndexVector;
  params.startingSfn = startingSfn;
