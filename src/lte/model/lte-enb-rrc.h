...
typedef std::map<uint8_t, std::vector<double>> CellCSIRSSinrMap;
...
public:
  ...
  void SetupUeFromHelper (uint8_t lteRnti);

  std::pair<uint8_t, uint8_t> GetCSIRSResourceVector (uint64_t imsi);

  void DoRecvRRCCSIRSReport (uint64_t imsi, uint16_t cellId, std::vector<double> csiRSVector);
...
private:
  ...
  void SetSweepParameters (bool sweepStarted, bool sweepCompleted);
...
private:
  ...
  void InitiateBeamSweepFromRrc (double perceivedSinr); 
  ...
  bool m_beamSweepStarted;    // Will be used at LTE coordinator
  bool m_beamSweepCompleted;  // Will be used at LTE coordinator
