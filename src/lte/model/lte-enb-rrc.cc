    ...
    m_carriersConfigured (false)
{
  ...
  m_beamSweepCompleted = false;
  m_beamSweepStarted = false;
}
...
LteEnbRrc::DoRecvUeSinrUpdate(EpcX2SapUser::UeImsiSinrParams params)
{
  ...
  //I don't see the purpose of m_cellSinrMap, since the important one is m_  
...
/*
      //Outage detection (commented out, since it interfered with the threshold based beam management)
      long double currentSinrDb = 10*std::log10((long double)currentSinr);
      if(currentSinrDb < m_outageThreshold)
      {
        NS_LOG_UNCOND("Outage detected! sinr="<<currentSinrDb<<" in cell "<<m_lastMmWaveCell[imsi]);
        NS_LOG_UNCOND("Imediate HO to cell "<<maxSinrCellId<<" needed with sinr="<<maxSinrDb);
      }
      if(currentSinrDb < 20 || prevSinrDb.find(m_lastMmWaveCell[imsi])->second-currentSinrDb>10)
      {
        NS_LOG_UNCOND("Perform Beamforming: IA, delay of 100ms");
      }
      prevSinrDb.find(m_lastMmWaveCell[imsi])->second = currentSinrDb;
*/
...
LteEnbRrc::TttBasedHandover(std::map<uint64_t, CellSinrMap>::iterator imsiIter, double sinrDifference, uint16_t maxSinrCellId, double maxSinrDb)
{
  ...
  if(alreadyAssociatedImsi && !onHandoverImsi)
  {
    ...
    else
    {
      ...
      if(maxSinrCellId != m_lastMmWaveCell[imsi] && (maxSinrDb - sinrDifference < 22.7))
      {
        ...
        if (m_rlmOn)
        {
          ...
          {
            if (m_imsiCellCSIRSSinrMap.at (imsi).find(m_lastMmWaveCell[imsi]) !=
                m_imsiCellCSIRSSinrMap.at (imsi).end ())
                {
                  auto csiReportNumber = m_imsiCellCSIRSSinrMap.at(imsi).at(m_lastMmWaveCell[imsi]).size ();
                  for (auto n = 0; n < csiReportNumber; n++)
                  {
                    if (maxSinrDb < 10 * log10(m_imsiCellCSIRSSinrMap.at(imsi).at(m_lastMmWaveCell[imsi]).at(n)))
                    {
        ...
        if (maxSNRHigherThanCSIRS)
        {
          if (!m_beamSweepCompleted)
          {
            if (!m_beamSweepStarted)
            {
              ...
              m_beamSweepTriggered = Simulator::ScheduleNow (
                      &LteEnbRrc::InitiateBeamSweepFromRrc, this, currentSinrDb);
              m_beamSweepStarted = true;
          ...
          else
          {
            for (auto const& iter : m_imsiBeamSweepCellSinrMap[imsi])
            {
              ...
              m_imsiCellSinrMap[imsi].at(iter.first) = iter.second;
            }
            ...
            if (m_imsiBeamSweepCellSinrMap[imsi].at(maxSweepSnrCellId) > 5)
            {
              if (maxSweepSnrCellId == maxSinrCellId)
                ...
                SetSweepParameters (false, false);
            }
            else
            {
              SetSweepParameters (false, false);
  ...
  if(handoverNeeded)
  {
    ...
    /*if(currentSinrDb < m_outageThreshold)
    {
      ...
      NS_LOG_UNCOND("Current Cell is in outage, handover in "<<millisecondsToHandover<<" ms");
    }*/
  ...
  */
  ...
  if(alreadyAssociatedImsi)
  {
    ...
    else
    {
      ...
      NS_LOG_INFO("## Warn: handover not triggered because the UE is already performing HO!");
    }
  }
  else
  {
...
LteEnbRrc::DoRecvCoordinatorHandoverRequest(EpcX2SapUser::SecondaryHandoverParams params)
{
  ...
  m_cmacSapProvider.at(0)->SetCSIRSFlag (true);
}
...
{
  m_imsiBeamSweepCellSinrMap.clear ();
  ...
  for (auto const& iter : msg.cellOptimalBeamMap)
  {
    if (m_imsiBeamSweepCellSinrMap.size () == 0)
    {
    ...
    if (m_realisticIA)
    {
      ...
      params.optimalBeamIndex = std::vector<uint16_t> ({
                iter.second.at(0).first.second,
                iter.second.at(1).first.second,
                iter.second.at(2).first.second,
                iter.second.at(3).first.second,
                iter.second.at(4).first.second,
                iter.second.at(5).first.second,
                iter.second.at(6).first.second,
                iter.second.at(7).first.second });
      
      if (maxCellId == iter.first)
      {
        params.maxSNRCell = true;
      }
      else
      {
        params.maxSNRCell = false;
      }
  ...
  if (m_beamSweepStarted)
  {
    m_beamSweepCompleted = true;
  }  
  //m_cphySapProvider.at(0)->SetOptimalGnbBeamForImsi(msg.ueImsi, msg.startingSfn, msg.optimalBeamIndex); 
}

void 
LteEnbRrc::DoRecvOptimalGnbBeamReport (LteRrcSap::OptimalGnbBeamReport params)
{
  m_cphySapProvider.at (0)->SetOptimalGnbBeamForImsi (params.ueImsi, params.startingSfn, params.optimalBeamIndex, params.isMaxSNRCell);
  if (params.isMaxSNRCell && m_rlmOn)
  {
    m_cmacSapProvider.at (0)->SetCSIRLMResources (m_csiPeriodicityInSlots, m_csiOffsetInSlots, m_maxCSIperFrame, params.ueImsi);
    m_cmacSapProvider.at (0)->SetCSIRSFlag (false);
  }  
}
...
LteEnbRrc::DoRecvDeRegisterUeCommand (LteRrcSap::DeRegisterUeContext params)
{
  ...
  outgoingParams.targetCellId = params.cellId;
...
{
  UeManager::State state = GetUeManager(GetRntiFromImsi (imsi))->GetState ();
...
LteEnbRrc::DoRecvDeregisterUe (LteRrcSap::DeRegisterUeContext params)
{
  ...
  case LteRrcSap::DeRegisterUeContext::SourceOfCommand::DeRegisterFromCoordinator:
    ...
    break;
  case LteRrcSap::DeRegisterUeContext::SourceOfCommand::DeRegisterFromUE:
    m_cphySapProvider.at(0)->DeregisterUeFromRRC (params.imsi);
    m_cmacSapProvider.at(0)->SetCSIRSFlag (true);
...
{
  m_cphySapProvider.at (0)->UpdateBeamsTBRLM (params.ueImsi, params.optimalBeamIndexVector, params.startingSfn);
}
...
void 
LteEnbRrc::SetupUeFromHelper (uint8_t lteRnti)
{
  m_lteRnti = lteRnti;
  LteEnbRrcSapUser::SetupUeParameters ueParams;
...
void
LteEnbRrc::InitiateBeamSweepFromRrc (double perceivedSinr)
{
  if (perceivedSinr < m_outageThreshold)
  {
    m_rrcSapUser->SendBeamSweepTriggerFromLTECoordinator (m_lteRnti);
  }
  else
  {
    m_rrcSapUser->SendBeamSweepCommandFromLTECoordinator (m_lteRnti);
  }
  Simulator::Schedule (m_beamSweepCompleteTimeoutDuration, &LteEnbRrc::BeamSweepTimer, this);
}

void 
LteEnbRrc::DoRecvUeDeRegisterCompletedFromGnb (EpcX2Sap::InformLteCoordinatorParams params)
{
  m_rrcSapUser->SendUeDeRegistrationCompletedUpdate (m_lteRnti, params.ueImsi);
}
...
LteEnbRrc::DoSendUeDeRegisterCompletedToLteCo (uint64_t imsi)
{
  if (m_lteCellId != 0)
  {
    ...
    updateParams.ueRnti = m_lteRnti;
    m_x2SapProvider->SendDeRegisterUeCompletedUpdate (updateParams);
  }
}

void 
LteEnbRrc::SetSweepParameters (bool sweepStarted, bool sweepCompleted)
{
  m_beamSweepStarted = sweepStarted;
  m_beamSweepCompleted = sweepCompleted;
}

void 
LteEnbRrc::DoRecvClearHandoverEvent (uint64_t imsi)
{
  HandoverEventMap::iterator handoverIt;
  for (handoverIt = m_imsiHandoverEventsMap.begin (); handoverIt != m_imsiHandoverEventsMap.end(); ++handoverIt)
  {
    handoverIt->second.scheduledHandoverEvent.Cancel ();
  }
  
  m_imsiHandoverEventsMap.clear ();
}
...
LteEnbRrc::DoRecvBeamPerformComplete (uint64_t imsi, bool beamSweepComplete)
{
  if (m_beamSweepStarted)
  {
    m_beamSweepCompleted = beamSweepComplete;
  }
}
...
void 
LteEnbRrc::BeamSweepTimer ()
{
  if (m_beamSweepStarted && !m_beamSweepCompleted)
  {
    SetSweepParameters (false, false);
    m_cmacSapProvider.at(0)->SetCSIRSFlag (false);
  }
...
void 
LteEnbRrc::DoRecvRRCCSIRSReport (uint64_t imsi, uint16_t cellId, std::vector<double> csiRSVector)
{
  CellCSIRSSinrMap cellCSIRSSinrMap;
  cellCSIRSSinrMap.insert (std::pair<uint8_t, std::vector<double>> (cellId, csiRSVector));

  m_imsiCellCSIRSSinrMap.clear ();  
...
LteEnbRrc::DoRelayRRCCSIRSReport (LteEnbCphySapUser::UeRRCCSIRSReport info) 
{
  ...
  params.csiRSVector = info.csiRSVector;
