...
ThreeGppPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ThreeGppPropagationLossModel")
    ...
    .AddAttribute ("ChannelTypeRaytracing",
                   "If raytracing channel is used the Pathloss needs to be 0, else from TR 38.900", 
                    BooleanValue(true),
                    MakeBooleanAccessor (&ThreeGppPropagationLossModel::m_channelTypeRaytracing),
                    MakeBooleanChecker ())
  ;
  return tid;
}
