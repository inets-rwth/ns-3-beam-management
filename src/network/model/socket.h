...
/**
 * \brief indicates the packet TX timestamp of the IP layer.
 */
class IpTimestampTag : public Tag
{
public:
  IpTimestampTag ();

  void SetIpTimestamp (double timestamp);
  double GetIpTimestamp (void) const;
  static TypeId GetTypeId (void);
  virtual TypeId GetInstanceTypeId (void) const;
  virtual uint32_t GetSerializedSize (void) const;
  virtual void Serialize (TagBuffer i) const;
  ...
  virtual void Print (std::ostream &os) const;
private:
  double m_timestamp;  //!< the timestamp carried by the tag
};

/**
 * \brief indicates the packet TX timestamp of the IP layer.
 */
class UdpTimestampTag : public Tag
{
public:
  UdpTimestampTag ();

  void SetUdpTimestamp (double timestamp);
  double GetUdpTimestamp (void) const;
  static TypeId GetTypeId (void);
  virtual TypeId GetInstanceTypeId (void) const;
  virtual uint32_t GetSerializedSize (void) const;
  virtual void Serialize (TagBuffer i) const;
